...
use App\Http\Resources\OJS\JournalCollection;
use App\Http\Resources\OJS\JournalResource;
...
class JournalRepository extends BaseRepository
{
    ...
    public function all(array $params = []): ResourceCollection
    {
        ...
        return new JournalCollection($data->paginate(isset($params['limit']) ? $params['limit'] : 25));
    }

    public function find(string $id): ?JsonResource
    {
        $data = $this->model->with('journalSettings')->find($id);
        if (is_null($data)) {
